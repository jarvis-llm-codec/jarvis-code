...
**이 문서는 AI 비서가 읽도록 쓰였습니다.** 사람이라면: 이 단계를 직접 따라 할
필요 없습니다. 이미 쓰고 있는, **컴퓨터에서 터미널 명령을 실행할 수 있는 AI** —
**Claude Code, Codex, Cursor, Windsurf** 같은 — 에게 이 페이지를 통째로 주고
이렇게 말하세요:

> *"jarvis-code 설치해줘. 이 안내서 그대로 따라 해. 내가 어떤 AI를 쓰는지
> 물어보고 거기에 맞게 최적으로 깔아주고, 무료 API 키 받는 것도 같이 해줘."*

작업은 AI가 합니다. 당신은 질문 몇 개에 답하고, 무료 가입 두 개를 클릭하면 됩니다.
