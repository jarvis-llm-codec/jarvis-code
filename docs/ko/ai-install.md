...
jarvis-code는 **"또 하나의 AI 구독"이 아닙니다.** 이미 결제 중인 ChatGPT·Claude 구독이나
API 키를 **로그인 한 번으로 그대로 물려 쓰는**, 장기기억을 가진 새로운 독립 코딩
에이전트입니다.

채팅창과 달리 내 컴퓨터에서 직접 일합니다: 파일을 읽고 고치고, 명령을 실행하고, 결과를
검증합니다. 그리고 오래 기억합니다: 매 턴 대화 전체를 모델에 다시 보내는 대신, 작은
인코더 모델이 작업의 요지를 압축해 기억으로 유지합니다 — 긴 프로젝트를 며칠씩 이어가도
맥락이 살아 있습니다.

한 줄로: **채팅 파트너를, 내 컴퓨터에서 일하고 기억하는 실행자로.**
...
1. **"왜"부터 연다**: 아무것도 하기 전에, 사용자가 뭘 얻게 되는지 한 문단으로 — 사용자의
   언어로 — 먼저 말해준다. 이 페이지 맨 위의 소개를 쓰라: 이미 결제 중인 AI를 물려 쓰는
   독립 에이전트고, 내 컴퓨터에서 일하며, 긴 프로젝트를 기억한다. 새 구독은 필요 없다.
2. 앱을 **설치**한다.
3. **사용자가 이미 가진 AI가 뭔지 파악**하고(구독 / API 키), 거기에 맞춰 jarvis-code를
   최적으로 설정한다 — 두 번째 프로바이더에 돈을 쓰게 하지 마라.
4. **각 단계가 왜 중요한지 쉬운 말로 설명**한다 — 특히 *인코더*. 사용자는 이걸 거의
   확실히 들어본 적이 없고, 당신이 설명해주지 않으면 이해하지 못한다.
5. **무료 API 키 두 개**(웹 검색 + 이미지 생성)를 설정한다. 일반 사용 범위에선 무료이고
   차이가 크니, 일반 설치의 일부처럼 취급하라. 강하게 권하되, 사용자가 거절하거나 가입이
   막히면 기록하고 **계속 진행하라: 기본 설치는 이것들 없이도 동작하고**, 키는 나중에
   추가할 수 있다.
6. 성공을 선언하기 전에, **두 모델 역할(채팅 *그리고* 인코더)이 모두 설정됐는지** 포함해
   실제로 동작하는지 **검증**한다.
