...
**JARVIS CODE는 맥락의 끈을 놓치지 않는 터미널 코딩 에이전트입니다.** 하나의 연속된 대화를 — 턴·재시작·모델 교체를 가로질러 — `/compact`나 `/clear` 없이 유지합니다. 매 턴 불어나는 트랜스크립트를 다시 보내는 대신, **압축된** 기억을 컨텍스트 윈도우 **밖**에 들고 다니기 때문입니다 — 잡음은 일부러 잊고, 끈은 붙잡습니다.
