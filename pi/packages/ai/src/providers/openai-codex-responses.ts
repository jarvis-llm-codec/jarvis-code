...
export const streamOpenAICodexResponses: StreamFunction<"openai-codex-responses", OpenAICodexResponsesOptions> = (
	...
	(async () => {
		...
		const showReasoningPlaceholder = () => {
			if (!options?.reasoningEffort || options.reasoningEffort === "none" || output.content.length > 0) {
				return;
			}
			startStream();
			const block = { type: "thinking" as const, thinking: "[thinking...]" };
...
async function processWebSocketStream(
	...
	try {
		...
		await processResponsesStream(
			startWebSocketOutputOnFirstEvent(mapCodexEvents(parseWebSocket(socket, options?.signal)), onStart, emitStart),
			output,
			stream,
			model,
			{
