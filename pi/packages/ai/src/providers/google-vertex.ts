...
export const streamGoogleVertex: StreamFunction<"google-vertex", GoogleVertexOptions> = (
	...
	(async () => {
		...
		const showReasoningPlaceholder = () => {
			if (!model.reasoning || !options?.thinking?.enabled || output.content.length > 0) return;
			startStream();
			...
			for await (const chunk of googleStream) {
				...
				if (candidate?.content?.parts) {
					for (const part of candidate.content.parts) {
						lastThoughtSignature = retainThoughtSignature(lastThoughtSignature, part.thoughtSignature);
						if (part.text !== undefined) {
							...
							if (
								...
								if (isThinking) {
									currentBlock = getReasoningPlaceholder(output) ?? null;
									if (!currentBlock) {
							...
							if (currentBlock.type === "thinking") {
								clearReasoningPlaceholder(currentBlock);
