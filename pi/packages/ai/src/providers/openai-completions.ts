...
import {
	clearReasoningPlaceholder,
	emitReasoningPlaceholder,
	getReasoningPlaceholder,
} from "./reasoning-placeholder.js";
...
export const streamOpenAICompletions: StreamFunction<"openai-completions", OpenAICompletionsOptions> = (
	...
	(async () => {
		...
		const showReasoningPlaceholder = () => {
			if (!options?.reasoningEffort || output.content.length > 0) return;
			startStream();
			emitReasoningPlaceholder(output, stream);
		};
		...
		try {
			...
			const ensureThinkingBlock = (thinkingSignature: string) => {
				if (!thinkingBlock) {
					thinkingBlock = getReasoningPlaceholder(output) ?? null;
					if (!thinkingBlock) {
						thinkingBlock = {
							type: "thinking",
							thinking: "",
				...
				clearReasoningPlaceholder(thinkingBlock);
...
function buildParams(
	...
	if (compat.thinkingFormat === "glm" && model.reasoning) {
...
function parseChunkUsage(
	...
	// OpenAI completion_tokens already includes reasoning_tokens, so split the
	// provider-reported reasoning tokens out for display while keeping totalTokens
	// billed correctly.
	const reasoningTokens = rawUsage.completion_tokens_details?.reasoning_tokens || 0;
	const totalCompletionTokens = rawUsage.completion_tokens || 0;
	const outputTokens = Math.max(0, totalCompletionTokens - reasoningTokens);
	const usage: AssistantMessage["usage"] = {
		...
		totalTokens: input + totalCompletionTokens + cacheReadTokens + cacheWriteTokens,
		cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 },
	};
