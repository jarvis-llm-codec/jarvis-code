...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|jarvis-code] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Update JARVIS Code and installed packages.

Options:
  --self                  Update JARVIS Code only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall JARVIS Code even if the current version is latest

Short forms:
  ${APP_NAME} update                Update JARVIS Code and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update jarvis-code    Update JARVIS Code only (self works as alias)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "pi" || source === "jarvis-code";
			if (sourceIsSelf) {
...
function isPiAgentSelfUpdateDisabled(): boolean {
	if (isTruthyEnvFlag(process.env.JARVIS_ALLOW_PI_AGENT_UPDATE)) return false;
	return isTruthyEnvFlag(process.env.JARVIS_DISABLE_PI_AGENT_UPDATE);
}

function printSelfUpdateBlocked(): void {
	console.error(`error: ${APP_NAME} self-update is disabled in this JARVIS Code fork.`);
	console.error(
		"Pi agent is pinned by the JARVIS Code repo/submodule; update it only through a reviewed JARVIS commit.",
	);
}
...
function printSelfUpdateUnavailable(npmCommand?: string[], updatePackageName = PACKAGE_NAME): void {
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of JARVIS Code executable: ${entrypoint}`);
	}
}
	...
	try {
		switch (options.command) {
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target) && isPiAgentSelfUpdateDisabled()) {
					printSelfUpdateBlocked();
					process.exitCode = 1;
					return true;
				}
