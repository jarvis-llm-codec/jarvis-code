...
function loadContextFileFromDir(dir: string): { path: string; content: string } | null {
	// JARVIS Code fork: do not fall back to CLAUDE.md / CLAUDE.MD —
	// those are Claude Code (the upstream Anthropic CLI) guides authored
	// for that agent, not for the Pi LLM running here. Loading them as
	// system context bleeds the wrong agent's policy into our turns.
	const candidates = ["JARVIS_REPO_CONTEXT.md", "JARVIS_REPO_CONTEXT.MD", "JARVIS.md", "JARVIS.MD"];
