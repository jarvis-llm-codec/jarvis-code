...
function isJarvisCompactionDisabled(): boolean {
	return isTruthyEnvFlag(process.env.JARVIS_DISABLE_COMPACTION);
}

function isJarvisAutoCompactionDisabled(): boolean {
	return isJarvisCompactionDisabled() || isTruthyEnvFlag(process.env.JARVIS_DISABLE_AUTO_COMPACTION);
}

function getJarvisRuntimeHistoryTurnLimit(): number | undefined {
	const raw = process.env.JARVIS_RUNTIME_HISTORY_TURNS?.trim();
...
function trimMessagesToRecentUserTurns(messages: AgentMessage[], maxUserTurns: number): AgentMessage[] {
	if (maxUserTurns <= 0) return messages;
	let userTurnsSeen = 0;
	let keepFromIndex = 0;
	for (let i = messages.length - 1; i >= 0; i--) {
		if (messages[i]?.role !== "user") continue;
		userTurnsSeen++;
		if (userTurnsSeen === maxUserTurns) {
			keepFromIndex = i;
		} else if (userTurnsSeen > maxUserTurns) {
			return keepFromIndex === 0 ? messages : messages.slice(keepFromIndex);
		}
	}
	return messages;
}

function applyJarvisRuntimeHistoryLimit(messages: AgentMessage[]): AgentMessage[] {
	const maxUserTurns = getJarvisRuntimeHistoryTurnLimit();
	if (!maxUserTurns) return messages;
	return trimMessagesToRecentUserTurns(messages, maxUserTurns);
}
...
const THINKING_LEVELS: ThinkingLevel[] = ["off", "minimal", "low", "medium", "high", "xhigh", "max", "ultra"];
...
export class AgentSession {
	...
	constructor(config: AgentSessionConfig) {
		...
		this._installJarvisRuntimeHistoryTransform();
		this.trimRuntimeHistoryForJarvis();
	}
	...
	/**
	 * JARVIS keeps durable history in session files and sidecar memory. The live
	 * Pi runtime state only needs a bounded recent-turn fallback.
	 */
	trimRuntimeHistoryForJarvis(): void {
		const messages = this.agent.state.messages;
		const trimmed = applyJarvisRuntimeHistoryLimit(messages);
		if (trimmed !== messages) {
			this.agent.state.messages = trimmed;
		}
	}

	private _installJarvisRuntimeHistoryTransform(): void {
		const originalTransformContext = this.agent.transformContext;
		this.agent.transformContext = async (messages, signal) => {
			const boundedInput = applyJarvisRuntimeHistoryLimit(messages);
			const transformed = originalTransformContext
				? await originalTransformContext(boundedInput, signal)
				: boundedInput;
			return applyJarvisRuntimeHistoryLimit(transformed);
		};
	}
	 ...
	 * Saves to the current session only if the level actually changes.
	 */
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		if (isJarvisCompactionDisabled()) {
			throw new Error("Context compaction is disabled in JARVIS Code.");
		}
	...
	private async _checkCompaction(assistantMessage: AssistantMessage, skipAbortedCheck = true): Promise<void> {
		if (isJarvisAutoCompactionDisabled()) return;
		...
		// Auto-compact threshold trigger disabled (v1.01): JHB encoder owns the context budget.
		// JARVIS runtime also sets JARVIS_DISABLE_COMPACTION to block manual and extension compaction entrypoints.
		// biome-ignore lint/correctness/noConstantCondition: intentional v1.01 disable, kept for re-enable
		if (false && shouldCompact(contextTokens, contextWindow, settings)) {
