	...
	return {
				...
				() =>
					new Promise<{
						...
						void (async () => {
							try {
								onUpdate?.({
									content: [{ type: "text", text: `Checking ${path}` }],
									details: undefined,
								});
								...
								onUpdate?.({
									content: [{ type: "text", text: `Preparing ${edits.length} edit block(s) for ${path}` }],
									details: undefined,
								});
								...
								onUpdate?.({
									content: [{ type: "text", text: `Writing updated ${path}` }],
									details: undefined,
								});
		...
		renderResult(result, options, theme, context) {
			if (options.isPartial) {
				...
				if (output) {
					component.addChild(new Spacer(1));
					component.addChild(new Text(theme.fg("muted", output), 1, 0));
				}
				return component;
			}
