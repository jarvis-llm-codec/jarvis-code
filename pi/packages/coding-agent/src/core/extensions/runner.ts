...
function cloneMessageForContextEvent(message: AgentMessage): AgentMessage {
	const cloned = { ...message } as AgentMessage;
	const content = (cloned as { content?: unknown }).content;
	if (Array.isArray(content)) {
		(cloned as { content?: unknown }).content = [...content];
	}
	return cloned;
}
...
export class ExtensionRunner {
	...
	async emitContext(messages: AgentMessage[]): Promise<AgentMessage[]> {
		const ctx = this.createContext();
		let currentMessages: AgentMessage[] | undefined;
		...
		for (const ext of this.extensions) {
			...
			for (const handler of handlers) {
				try {
					currentMessages ??= messages.map(cloneMessageForContextEvent);
