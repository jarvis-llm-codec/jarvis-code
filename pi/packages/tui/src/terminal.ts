...
export function shouldEnableMouseReporting(env: NodeJS.ProcessEnv = process.env): boolean {
	const value = env.PI_TUI_MOUSE_REPORTING ?? env.PI_TUI_MOUSE_WHEEL ?? env.PI_TUI_MOUSE;
...
export class ProcessTerminal implements Terminal {
	...
	start(onInput: (data: string) => void, onResize: () => void): void {
		...
		// Mouse reporting captures terminal selection/scroll. Keep it opt-in so
		// users can select and copy chat text with the terminal's native behavior.
